...
const scrapeAmazon = async () => {
  try {
    ...
    for (let i = 0; i <= categories.length - 1; i++) {
      let fetchedProducts = await getBestSellersPerCategory(categories[parseInt(i)].categoryID);
    ...
    const dur = moment.duration(moment(finishTime, "HH:mm:ss") - moment(startTime, "HH:mm:ss"));
    console.log(`Finish scrape Amazon Best Sellers at ${finishTime}. Elapsed Time: ${dur.minutes()}:${dur.seconds()}`);
    return products;
  } catch (e) {
    console.log("TODO: Custom Error Handling");
  }
